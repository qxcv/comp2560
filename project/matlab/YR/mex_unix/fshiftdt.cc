...
//      a,b,dest_shift,dest_length,dest_step)
   //dt1d(vals+x*sizy, tmpM+x*leny, tmpIy+x*leny, 1, sizy, ay, by, offy, leny, step, fvals+x*sizy);
void dt1d(double *src, double *dst, int *ptr, int step, int len, double a, double b, int dshift, int dlen, double dstep, double *flow) {
  int   *v = new int[len];
  ...
  for (q = 1; q <= len-1; q++) {
    float s = ((src[q*step] - src[v[k]*step]) - b*(q+flow[q*step] - v[k]) + a*(square(q+flow[q*step]) - square(v[k]))) / (2*a*(q+flow[q*step]-v[k]));
    while (s <= z[k]) {
      k--;
      s  = ((src[q*step] - src[v[k]*step]) - b*(q+flow[q*step] - v[k]) + a*(square(q+flow[q*step]) - square(v[k]))) / (2*a*(q+flow[q*step]-v[k]));
    }
    k++;
    v[k]   = q;
    z[k]   = s;
   ...
   for (int i=0; i <= dlen-1; i++) {
     while (z[k+1] < q)
       k++;
     dst[i*step] = a*square(q+flow[q*step]-v[k]) + b*(q+flow[q*step]-v[k]) + src[v[k]*step];
     ptr[i*step] = v[k];
     q += dstep;
  }
...
void mexFunction(int nlhs, mxArray *plhs[], int nrhs, const mxArray *prhs[]) { 
  ...
  double *fvals = (double *)mxGetPr(prhs[10]); // this incorporates the flow vectors.

mexPrintf("Hello world");
/*
  for (unsigned int xx=0; xx<lenx; xx++){
     for (unsigned int yy=0; yy<leny;yy++){
        mexPrintf("%lf ", *(vals+leny*xx+lenx));
     }
     mexPrintf("\n");
  }
*/
  ...
  for (int y = 0; y < leny; y++)
    dt1d(tmpM+y, M+y, Ix+y, leny, sizx, ax, bx, offx, lenx, step, fvals+y);
