// mex file to compute the full HOG on a 2D matrix of optical flow vectors.
// calling convention is h2=flow_features(flow_vecs,sbin); where flow_vecs is nxmx2 double matrix and sbin is the bin size.
// output: HOG descriptors for each cell. 
#include <math.h>
#include "mex.h"
...
// main function:
// takes a flow image:  u=fim
// and bin size 
// returns MBH features
...
mxArray *process(const mxArray *mxfimage, const mxArray *mxsbin) {
  ...
  if (mxGetNumberOfDimensions(mxfimage) != 2 ||
